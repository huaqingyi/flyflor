...
If you discover a security vulnerability in flyflor, please report it by:
...
chmod 600 ~/.flyflor/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.flyflor/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.flyflor/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run flyflor with a dedicated user account
...
- Keep authentication data in `~/.flyflor/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade flyflor-ai
```
...
- Subscribe to security advisories for flyflor and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install flyflor-ai
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash flyflor
   sudo -u flyflor flyflor gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.flyflor
   chmod 600 ~/.flyflor/config.json
   chmod 700 ~/.flyflor/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.flyflor/logs/flyflor.log
   ```
...
- **Chat history is stored locally** - protect the `~/.flyflor` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.flyflor/logs/flyflor.log
   ```
...
- GitHub Security Advisories: https://github.com/flyflor-ai/flyflor/security/advisories
- Release Notes: https://github.com/flyflor-ai/flyflor/releases
