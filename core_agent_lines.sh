...
echo "flyflor line count"
echo "=================="
echo ""

echo "Core runtime"
echo "------------"
core_agent=$(count_top_level_py_lines "flyflor/agent")
core_bus=$(count_top_level_py_lines "flyflor/bus")
core_config=$(count_top_level_py_lines "flyflor/config")
core_cron=$(count_top_level_py_lines "flyflor/cron")
core_heartbeat=$(count_top_level_py_lines "flyflor/heartbeat")
core_session=$(count_top_level_py_lines "flyflor/session")
...
echo "----------------"
extra_tools=$(count_recursive_py_lines "flyflor/agent/tools")
extra_skills=$(count_skill_lines "flyflor/skills")
extra_api=$(count_recursive_py_lines "flyflor/api")
extra_cli=$(count_recursive_py_lines "flyflor/cli")
extra_channels=$(count_recursive_py_lines "flyflor/channels")
extra_utils=$(count_recursive_py_lines "flyflor/utils")
...
echo "Notes"
echo "-----"
echo "  - agent/ only counts top-level Python files under flyflor/agent"
echo "  - tools/ is counted separately from flyflor/agent/tools"
echo "  - skills/ counts .md, .py, and .sh files"
echo "  - not included here: command/, providers/, security/, templates/, flyflor.py, root files"
