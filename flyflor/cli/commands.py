...
# Remove default handler and re-add with unified flyflor format
logger.remove()
...
from flyflor import __logo__, __version__
...
from flyflor.cli.stream import StreamRenderer, ThinkingSpinner
from flyflor.config.paths import get_workspace_path, is_default_workspace
from flyflor.config.schema import Config
from flyflor.utils.helpers import sync_workspace_templates
from flyflor.utils.restart import (
...
app = typer.Typer(
    ...
    help=f"{__logo__} flyflor - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from flyflor.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} flyflor[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} flyflor[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} flyflor v{__version__}")
        raise typer.Exit()
...
def onboard(
    ...
    """Initialize flyflor configuration and workspace."""
    from flyflor.config.loader import get_config_path, load_config, save_config, set_config_path
    from flyflor.config.schema import Config
    ...
    if wizard:
        from flyflor.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'flyflor onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'flyflor agent -m "Hello!"'
    gateway_cmd = "flyflor gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} flyflor is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/flyflor-ai/flyflor#-chat-apps[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from flyflor.channels.registry import discover_all
...
def _make_provider(config: Config):
    ...
    """
    from flyflor.providers.factory import make_provider
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from flyflor.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from flyflor.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from flyflor.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show flyflor runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'flyflor-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from flyflor.agent.loop import AgentLoop
    from flyflor.api.server import create_app
    from flyflor.bus.queue import MessageBus
    from flyflor.session.manager import SessionManager

    if verbose:
        logger.enable("flyflor")
    else:
        logger.disable("flyflor")
    ...
    agent_loop = AgentLoop(
        ...
        blackboard_config=runtime_config.blackboard,
...
def _run_gateway(
    ...
    *,
    ...
    from flyflor.agent.loop import AgentLoop
    from flyflor.agent.tools.cron import CronTool
    from flyflor.agent.tools.message import MessageTool
    from flyflor.bus.queue import MessageBus
    from flyflor.channels.manager import ChannelManager
    from flyflor.cron.service import CronService
    from flyflor.cron.types import CronJob
    from flyflor.heartbeat.service import HeartbeatService
    from flyflor.providers.factory import build_provider_snapshot, load_provider_snapshot
    from flyflor.session.manager import SessionManager

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} Starting flyflor gateway version {__version__} on port {port}...")
    ...
    from flyflor.agent.loop import UNIFIED_SESSION_KEY
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from flyflor.utils.evaluator import evaluate_response
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show flyflor runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from flyflor.channels.registry import discover_all
    from flyflor.config.loader import load_config, set_config_path
...
def _get_bridge_dir() -> Path:
    ...
    from flyflor.config.paths import get_bridge_install_dir

    user_bridge = get_bridge_install_dir()
    stamp_file = user_bridge / ".flyflor-bridge-source-hash"

    # Find source bridge: first check package data, then source dir
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # flyflor/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall flyflor")
        raise typer.Exit(1)
...
def plugins_list():
    ...
    from flyflor.channels.registry import discover_all, discover_channel_names
    from flyflor.config.loader import load_config
...
def status():
    """Show flyflor status."""
    from flyflor.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} flyflor Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from flyflor.providers.registry import PROVIDERS
...
def _login_github_copilot() -> None:
    try:
        from flyflor.providers.github_copilot_provider import login_github_copilot
