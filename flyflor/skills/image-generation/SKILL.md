...
If the `generate_image` tool is not available in the current tool list, tell the user that image generation is not enabled for this flyflor instance.
...
The tool stores generated images as persistent artifacts under flyflor's media directory and returns structured metadata:
...
```text
generate_image(
  prompt="A minimal app icon for Flyflor: white-haired flower-crystal assistant, violet and cyan glow, rounded square, no text",
  aspect_ratio="1:1",
  image_size="1K"
)
```
...
```text
generate_image(
  prompt="Use the reference image. Keep the same Flyflor character and composition, but change the palette to warm orange and add a subtle sunrise background.",
  reference_images=["/home/user/.flyflor/media/generated/2026-05-08/img_ab12cd34ef56.png"],
  aspect_ratio="1:1",
  image_size="1K"
)
```
