---
name: update-setup
description: One-time setup wizard for the flyflor upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `flyflor --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v flyflor || true
python -m pip show flyflor-ai || true
pipx list | sed -n '/flyflor-ai/,+3p' || true
uv tool list | sed -n '/flyflor-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how flyflor was
installed.
...
`question: "Where is your flyflor source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "flyflor-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "flyflor-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "flyflor-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade flyflor to the latest version. Triggers: upgrade flyflor, update flyflor, 升级flyflor, 更新flyflor."
---
...
4. Use `exec` to verify: `flyflor --version`
5. Tell the user the new version. Say: "Run `/restart` to restart flyflor and apply the update. If `/restart` is unavailable in this channel, restart the flyflor process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade flyflor' when you want to update."
