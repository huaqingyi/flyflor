...
from flyflor.cron.types import CronSchedule
...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.flyflor/workspace"
...
class ProvidersConfig(Base):
    ...
    fastai: ProviderConfig = Field(default_factory=ProviderConfig)
...
class BlackboardConfig(Base):
    """Blackboard discussion mode configuration."""

    enabled: bool = True
    mode: Literal["direct", "direct-with-watch", "blackboard"] = "blackboard"
    show_discussion: bool = True
    direct_threshold: float = Field(default=0.35, ge=0.0, le=1.0)
    threshold: float = Field(default=0.55, ge=0.0, le=1.0)
    allow_auto_escalation: bool = True


class Config(BaseSettings):
    """Root configuration for flyflor."""
    ...
    model_config = ConfigDict(env_prefix="FLYFLOR_", env_nested_delimiter="__")
