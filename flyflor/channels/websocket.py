"""WebSocket server channel: flyflor acts as a WebSocket server and serves connected clients."""
...
from flyflor.command.builtin import builtin_command_palette
from flyflor.config.paths import get_media_dir
from flyflor.config.schema import Base
from flyflor.utils.helpers import safe_filename
from flyflor.utils.media_decode import (
    FileSizeExceeded,
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      flyflor and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-Flyflor-Auth: <secret>``.
...
def _read_webui_blackboard_mode() -> str:
    """Return the configured blackboard mode for readonly webui display."""
    try:
        from flyflor.config.loader import load_config

        cfg = load_config().blackboard
        return cfg.mode if cfg.enabled else "direct"
    except Exception as e:
        logger.debug("webui bootstrap could not load blackboard mode: {}", e)
        return "blackboard"
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-Flyflor-Auth") or headers.get("x-flyflor-auth")
    if not header_token:
        return False
...
class WebSocketChannel(BaseChannel):
        ...
        return _http_json_response(
            {
                ...
                "blackboard_mode": _read_webui_blackboard_mode(),
            }
        )
    ...
    def _settings_payload(self, *, requires_restart: bool = False) -> dict[str, Any]:
        ...
        from flyflor.providers.registry import PROVIDERS, find_by_name
        ...
        return {
            "agent": {
                ...
                "blackboard_mode": config.blackboard.mode if config.blackboard.enabled else "direct",
            },
            "providers": [
    ...
    def _handle_settings_update(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from flyflor.config.loader import load_config, save_config
