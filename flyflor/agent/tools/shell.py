...
from flyflor.agent.tools.sandbox import wrap_command
from flyflor.agent.tools.schema import IntegerSchema, StringSchema, tool_parameters_schema
...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (deny_patterns or []) + [
            ...
            r":\(\)\s*\{.*\};\s*:",          # fork bomb
            # Block writes to flyflor internal state files (#2989).
    ...
    async def execute(
        ...
        if self.path_append:
            if _IS_WINDOWS:
                ...
                env["FLYFLOR_PATH_APPEND"] = self.path_append
                command = f'export PATH="$PATH{os.pathsep}$FLYFLOR_PATH_APPEND"; {command}'
    ...
    def _guard_command(self, command: str, cwd: str) -> str | None:
        ...
        from flyflor.security.network import contains_internal_url
