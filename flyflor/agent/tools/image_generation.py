...
from flyflor.agent.tools.schema import (
    ArraySchema,
    IntegerSchema,
    StringSchema,
...
from flyflor.config.schema import ImageGenerationToolConfig
from flyflor.providers.image_generation import (
    AIHubMixImageGenerationClient,
    ImageGenerationError,
    OpenRouterImageGenerationClient,
)
from flyflor.utils.artifacts import (
    ArtifactError,
    generated_image_tool_result,
    store_generated_image_artifact,
)
from flyflor.utils.helpers import detect_image_mime

if TYPE_CHECKING:
    from flyflor.config.schema import ProviderConfig
    ...
    def _resolve_reference_image(self, value: str) -> str:
        ...
        if not any(_is_relative_to(resolved, root) for root in allowed_roots):
            raise ImageGenerationError(
                "reference_images must be inside the workspace or flyflor media directory"
            )
