...
from flyflor.providers.base import LLMProvider, LLMResponse, ToolCallRequest
from flyflor.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/flyflor-ai/flyflor",
    "X-OpenRouter-Title": "flyflor",
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("FLYFLOR_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply Flyflor attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _should_use_responses_api(
        ...
        if self._spec and self._spec.name not in ("openai", "github_copilot", "fastai"):
            return False
        if self._spec is None or self._spec.name not in ("github_copilot", "fastai"):
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("FLYFLOR_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
