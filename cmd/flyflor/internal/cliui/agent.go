...
// AgentProgress renders compact worker-turn progress for the terminal.
...
func RenderAgentIntro(model, session string) string {
	...
	runtime = append(runtime,
		...
		kvKeyStyle().Render("流程")+"     "+progressBar(38, 10)+" "+mutedStyle().Render("scheduler 就绪"),
	)

	graph := []string{
		panelTitleStyle(accentViolet).Render("Blackboard") + mutedStyle().Render(" 调度"),
		mutedStyle().Render("  │"),
		panelTitleStyle(accentCyan).Render("Flyflor Planner") + mutedStyle().Render(" 拆解/执行") + mutedStyle().Render("  ⇄  ") + panelTitleStyle(accentPink).Render("Flyflor Reviewer") + mutedStyle().Render(" 复核"),
	}
...
func RenderAgentStatusBar(session, model string) string {
	parts := []string{
		...
		panelTitleStyle(accentPink).Render("Workers") + " planner/reviewer",
...
func RenderBlackboardTabs(turns []BlackboardTurn, selected int) string {
	...
	rows := []string{
		...
		blackboardLeft("A", "Flyflor Planner", "拆解任务边界、执行路径和需要验证的点。", width),
		blackboardRight("B", "Flyflor Reviewer", "复核风险、遗漏和用户是否能读懂本轮结果。", width),
...
func renderBlackboardPanel(content string, consensus bool, width int) string {
	...
	if consensus {
		rows = append(rows,
			...
			blackboardLeft("A", "Flyflor Planner", "我确认交付内容已经覆盖本轮问题；如果有工具结果，它们应能支撑最终回答。", width),
			blackboardRight("B", "Flyflor Reviewer", "我完成复核：黑板保留本轮关键判断，而不是只显示一句“达成共识”。", width),
		...
		rows = append(rows,
			...
			blackboardLeft("A", "Flyflor Planner", "我先拆任务：明确用户想完成什么，再判断需要设计、改代码、跑工具还是直接回答。", width),
			blackboardRight("B", "Flyflor Reviewer", "我负责复核：检查计划是否答偏、是否遗漏限制、最终结果是否能被用户读懂。", width),
			blackboardCenter("记忆", "上下文 · 读取 Flyflor 身份、用户偏好、最近任务和可复用经验。", width),
			blackboardLeft("A", "Flyflor Planner", "执行计划：先处理最影响体验的问题，再做配套调整，最后用测试或截图验证。", width),
			blackboardRight("B", "Flyflor Reviewer", "阅读标准：每条黑板记录都说明“为什么出现、谁负责、下一步是什么”。", width),
