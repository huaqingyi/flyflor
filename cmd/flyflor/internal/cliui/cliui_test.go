...
func TestRenderRootHelpPlainPanelIncludesTUIWorkflow(t *testing.T) {
	out := RenderRootHelp("flyflor", "flyflor [flags]")
	for _, want := range []string{"Flyflor CLI / 帮助", "TUI 工作流", "Ctrl+S", "/yolo", "fx", "gum"} {
		if !findSubstring(out, want) {
			t.Fatalf("root help missing %q:\n%s", want, out)
		}
	}
	for _, line := range strings.Split(strings.TrimRight(out, "\n"), "\n") {
		if got := lipgloss.Width(line); got > 78 {
			t.Fatalf("root help line width = %d, want <= 78: %q\n%s", got, line, out)
		}
	}
}
