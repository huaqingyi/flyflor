...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from OpenClaw-style homes into Flyflor",
		Args:  cobra.NoArgs,
		Example: `  flyflor migrate
  flyflor migrate --from openclaw
  flyflor migrate --dry-run
  flyflor migrate --refresh
  flyflor migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.picoclaw / $PICOCLAW_HOME)")
