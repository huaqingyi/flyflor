...
func printOnboardPlain(logo string, encrypt bool, configPath string) {
	...
	fmt.Println("\nNext steps:")
	if encrypt {
		fmt.Println("  1. Set your encryption passphrase before starting Flyflor:")
	...
	fmt.Println("")
	if encrypt {
		fmt.Println("  3. Chat: flyflor agent -m \"Hello!\"")
	} else {
		fmt.Println("  2. Chat: flyflor agent -m \"Hello!\"")
	}
}
...
func printOnboardFancy(logo string, encrypt bool, configPath string) {
	...
	ready := titleBarStyle().Render(logo+" Flyflor is ready!") + "\n"
	fmt.Println()
...
func buildOnboardingSteps(encrypt bool, configPath string) string {
	...
	if encrypt {
		b.WriteString("1. Set your encryption passphrase before starting Flyflor:\n")
