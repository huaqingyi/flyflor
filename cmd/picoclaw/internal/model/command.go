...
available list online and pick one), use the 'add' subcommand:
  ...
  flyflor model add --help

Examples:
  flyflor model                    # Show current default model
  flyflor model gpt-5.2           # Set gpt-5.2 as default
  flyflor model claude-sonnet-4.6 # Set claude-sonnet-4.6 as default
  flyflor model local-model       # Set local VLLM server as default
  flyflor model add -b URL -k KEY # Add a model from a custom endpoint
...
(running at localhost:8000 by default) which does not require an API key.`,
		...
		ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			...
			if _, err := os.Stat(configPath); err != nil {
				return []string{LocalModel}, cobra.ShellCompDirectiveNoFileComp
			}
			cfg, err := config.LoadConfig(configPath)
			if err != nil {
				return nil, cobra.ShellCompDirectiveNoFileComp
			}
			candidates := make([]string, 0, len(cfg.ModelList)+1)
			for _, model := range cfg.ModelList {
				if model == nil || !model.Enabled || strings.TrimSpace(model.ModelName) == "" {
					continue
				}
				candidates = append(candidates, model.ModelName)
			}
			candidates = append(candidates, LocalModel)
...
func showCurrentModel(cfg *config.Config) {
	...
	fmt.Println("\nTip: 'flyflor model add -b URL -k KEY' adds a model from a custom")
	fmt.Println("     OpenAI-compatible endpoint (see 'flyflor model add --help').")
}
