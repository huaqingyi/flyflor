...
const externalFrontendDistEnv = "FLYFLOR_WEB_DIST_DIR"
...
func registerEmbedRoutes(mux *http.ServeMux) {
	...
	var subFS fs.FS
	if externalDist := strings.TrimSpace(os.Getenv(externalFrontendDistEnv)); externalDist != "" {
		if _, err := os.Stat(externalDist); err != nil {
			logger.WarnC("web", fmt.Sprintf("Warning: %s=%q is not readable: %v", externalFrontendDistEnv, externalDist, err))
		} else {
			logger.InfoC("web", fmt.Sprintf("Serving frontend from %s=%q", externalFrontendDistEnv, externalDist))
			subFS = os.DirFS(externalDist)
		}
	}
