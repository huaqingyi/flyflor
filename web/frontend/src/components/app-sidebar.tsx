...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  return (
    <Sidebar
      {...props}
      className="bg-background/95 border-r-border/40 border-r pt-3"
    >
      <SidebarContent className="bg-background/95">
        <div className="mx-3 mb-3 rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-3 py-3">
          <div className="text-xs font-semibold">Flyflor Cockpit</div>
          <div className="text-muted-foreground mt-1 text-[11px] leading-relaxed">
            多智能体运行时、bridge 黑板、记忆与工具事件控制台。
          </div>
        </div>
        {navGroups.map((group) => (
          <Collapsible
