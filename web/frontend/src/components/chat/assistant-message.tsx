      ...
      {(hasText || isCollapsedBlock || hasToolCalls) && (
        <div
          className={cn(
            ...
            isThought
              ? "border-violet-400/30 bg-violet-500/5 text-muted-foreground"
              : isCollapsedBlock
        ...
        >
          {isCollapsedBlock && (
            <div
              className="text-muted-foreground/70 hover:text-muted-foreground/90 flex cursor-pointer items-center justify-between px-3 py-2 text-[12px] font-medium transition-colors select-none"
          ...
          {(!isCollapsedBlock || isExpanded) && !isToolCalls && hasText && (
            <div
              className={cn(
                ...
                isThought
                  ? "prose-p:my-1 prose-p:whitespace-pre-wrap max-h-24 overflow-hidden px-3 pt-0 pb-3 text-[12px] leading-relaxed opacity-75"
