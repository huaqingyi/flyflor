  ...
  hydratePromise = loadSessionMessages(storedSessionId)
    .then((historyMessages) => {
      ...
      updateChatStore({
        ...
        blackboardEntries: deriveBlackboardEntries(historyMessages),
        isTyping: false,
        hasHydratedActiveSession: true,
      })
    })
...
export function sendChatMessage({
  ...
  const blackboardEntries = buildBlackboardEntriesForUserMessage({
    messageId: id,
  ...
  try {
    ...
    updateChatStore((prev) => ({
      ...
      blackboardEntries: prev.blackboardEntries.filter(
        (entry) => entry.relatedMessageId !== id,
      ),
      isTyping: false,
    }))
    return false
