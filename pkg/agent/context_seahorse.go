...
type seahorseContextManager struct {
	...
	semanticMemory *semanticmemory.Manager
}
...
func newSeahorseContextManager(raw json.RawMessage, al *AgentLoop) (ContextManager, error) {
	...
	semanticCfg := semanticmemory.ConfigFromRaw(raw)
	if semanticCfg.Enabled {
		initCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		semanticMgr, semErr := semanticmemory.New(initCtx, semanticCfg)
		cancel()
		if semErr != nil {
			logger.WarnCF("memory", "Qdrant semantic memory is configured but unavailable", map[string]any{
				"url":        semanticCfg.QdrantURL,
				"collection": semanticCfg.Collection,
				"error":      semErr.Error(),
			})
		} else {
			mgr.semanticMemory = semanticMgr
			logger.InfoCF("memory", "Qdrant semantic memory enabled", map[string]any{
				"url":        semanticCfg.QdrantURL,
				"collection": semanticCfg.Collection,
				"dimensions": semanticCfg.Dimensions,
				"embedding":  semanticCfg.Embedding.Provider,
			})
		}
	}
...
func (m *seahorseContextManager) Assemble(ctx context.Context, req *AssembleRequest) (*AssembleResponse, error) {
	...
	if m.semanticMemory != nil && strings.TrimSpace(req.Query) != "" {
		searchCtx, cancel := context.WithTimeout(ctx, 4*time.Second)
		hits, searchErr := m.semanticMemory.Search(searchCtx, req.Query)
		cancel()
		if searchErr != nil {
			logger.WarnCF("memory", "Qdrant semantic recall failed", map[string]any{
				"session": req.SessionKey,
				"error":   searchErr.Error(),
			})
		} else if formatted := semanticmemory.FormatHits(hits); formatted != "" {
			if strings.TrimSpace(summary) != "" {
				summary += "\n\n" + formatted
			} else {
				summary = formatted
			}
			logger.InfoCF("memory", "Qdrant semantic recall injected", map[string]any{
				"session": req.SessionKey,
				"hits":    len(hits),
			})
		}
	...
	// Semantic memory is appended as another context block so BuildMessages can
	// inject it into the system prompt without changing provider message shape.
...
func (m *seahorseContextManager) Ingest(ctx context.Context, req *IngestRequest) error {
	...
	if m.semanticMemory != nil {
		indexCtx, cancel := context.WithTimeout(ctx, 4*time.Second)
		count, semErr := m.semanticMemory.IndexMessage(indexCtx, req.SessionKey, semanticmemory.MessageInput{
			Role:    req.Message.Role,
			Content: req.Message.Content,
		})
		cancel()
		if semErr != nil {
			logger.WarnCF("memory", "Qdrant semantic indexing failed", map[string]any{
				"session": req.SessionKey,
				"role":    req.Message.Role,
				"error":   semErr.Error(),
			})
		} else if count > 0 {
			logger.InfoCF("memory", "Qdrant semantic memories indexed", map[string]any{
				"session": req.SessionKey,
				"role":    req.Message.Role,
				"count":   count,
			})
...
func (m *seahorseContextManager) Clear(ctx context.Context, sessionKey string) error {
	...
	if m.semanticMemory != nil {
		if err := m.semanticMemory.DeleteSession(ctx, sessionKey); err != nil {
			logger.WarnCF("memory", "Qdrant semantic session delete failed", map[string]any{
				"session": sessionKey,
				"error":   err.Error(),
			})
		}
...
func (m *seahorseContextManager) bootstrapSession(ctx context.Context, sessionKey string) {
	...
	if m.semanticMemory != nil {
		...
		for _, h := range history {
			indexCtx, cancel := context.WithTimeout(ctx, 4*time.Second)
			count, err := m.semanticMemory.IndexMessage(indexCtx, sessionKey, semanticmemory.MessageInput{
				Role:    h.Role,
				Content: h.Content,
			})
			cancel()
			if err != nil {
				logger.WarnCF("memory", "Qdrant semantic bootstrap failed", map[string]any{
					"session": sessionKey,
					"role":    h.Role,
					"error":   err.Error(),
				})
		...
		if indexed > 0 {
			logger.InfoCF("memory", "Qdrant semantic bootstrap indexed", map[string]any{
				"session": sessionKey,
				"count":   indexed,
			})
		}
