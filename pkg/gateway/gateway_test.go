...
func TestShutdownGatewayClosesMessageBus(t *testing.T) {
	...
	shutdownGateway(&services{HealthServer: healthServer}, al, &startupBlockedProvider{reason: "not used"}, msgBus, true)
	...
	if w.Code != http.StatusServiceUnavailable {
		t.Fatalf("/ready after shutdown status = %d, want %d", w.Code, http.StatusServiceUnavailable)
	}
}
