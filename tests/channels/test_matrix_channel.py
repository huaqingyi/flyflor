...
from flyflor.channels.matrix import _build_matrix_text_content

import flyflor.channels.matrix as matrix_module
from flyflor.bus.events import OutboundMessage
from flyflor.bus.queue import MessageBus
from flyflor.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from flyflor.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("flyflor.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "flyflor.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("flyflor.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "flyflor.channels.matrix.asyncio.create_task", _fake_create_task
    )
