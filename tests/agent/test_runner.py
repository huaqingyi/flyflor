...
from flyflor.agent.tools.base import Tool
...
def _make_loop(tmp_path):
    ...
    with patch("flyflor.agent.loop.ContextBuilder"), \
         patch("flyflor.agent.loop.SessionManager"), \
         patch("flyflor.agent.loop.SubagentManager") as MockSubMgr:
...
async def test_runner_preserves_reasoning_fields_and_tool_results():
    from flyflor.agent.runner import AgentRunSpec, AgentRunner
...
async def test_runner_calls_hooks_in_order():
    from flyflor.agent.hook import AgentHook, AgentHookContext
    from flyflor.agent.runner import AgentRunSpec, AgentRunner
...
def test_is_ssrf_violation_recognizes_private_url_blocks():
    ...
    from flyflor.agent.runner import AgentRunner
...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".flyflor" / "tool-results" / "test_runner" / "call_big.txt").exists()


def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from flyflor.utils.helpers import maybe_persist_tool_result

    root = tmp_path / ".flyflor" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "flyflor.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "flyflor.utils.helpers.logger.exception",
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("flyflor.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "flyflor.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("flyflor.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_llm_error_not_appended_to_session_messages():
    ...
    from flyflor.agent.runner import (
        AgentRunSpec,
        AgentRunner,
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from flyflor.agent.loop import AgentLoop
    from flyflor.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from flyflor.bus.events import InboundMessage
    from flyflor.bus.queue import MessageBus
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("flyflor.agent.tools.filesystem.ListDirTool.execute", fake_execute)
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from flyflor.agent.runner import AgentRunSpec, AgentRunner, _MAX_LENGTH_RECOVERIES
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from flyflor.agent.runner import AgentRunner, _BACKFILL_CONTENT
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from flyflor.agent.loop import AgentLoop
    from flyflor.agent.runner import _BACKFILL_CONTENT
    from flyflor.bus.events import InboundMessage
    from flyflor.bus.queue import MessageBus
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from flyflor.agent.runner import AgentRunSpec, AgentRunner, _BACKFILL_CONTENT
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from flyflor.agent.runner import AgentRunner, _MICROCOMPACT_KEEP_RECENT
...
async def test_drain_injections_passes_limit_to_callback_when_supported():
    ...
    from flyflor.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTIONS_PER_TURN
    from flyflor.bus.events import InboundMessage
...
async def test_injection_cycles_capped_at_max():
    ...
    from flyflor.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTION_CYCLES
    from flyflor.bus.events import InboundMessage
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from flyflor.bus.events import InboundMessage
    from flyflor.bus.queue import MessageBus
    from flyflor.agent.runner import _MAX_INJECTIONS_PER_TURN
...
async def test_drain_injections_on_empty_final_response():
    ...
    from flyflor.agent.runner import AgentRunSpec, AgentRunner, _MAX_EMPTY_RETRIES
    from flyflor.bus.events import InboundMessage
...
async def test_drain_injections_set_flag_when_followup_arrives_after_last_iteration():
    ...
    from flyflor.agent.hook import AgentHook
    from flyflor.agent.runner import AgentRunSpec, AgentRunner
    from flyflor.bus.events import InboundMessage
...
def test_snip_history_preserves_user_message_after_truncation(monkeypatch):
    ...
    monkeypatch.setattr("flyflor.agent.runner.estimate_prompt_tokens_chain", lambda *_a, **_kw: (500, None))
