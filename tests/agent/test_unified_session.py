...
from flyflor.command.builtin import cmd_new, register_builtin_commands
from flyflor.command.router import CommandContext, CommandRouter
from flyflor.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("flyflor.agent.loop.SessionManager"), \
         patch("flyflor.agent.loop.SubagentManager") as MockSubMgr, \
         patch("flyflor.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates flyflor onboard)."""
        from flyflor.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from flyflor.agent.memory import Consolidator, MemoryStore
