"""Tests for the Flyflor programmatic facade."""
...
from flyflor.flyflor import Flyflor, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Flyflor.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Flyflor.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("flyflor.config.loader.load_config") as mock_load, \
         patch("flyflor.flyflor._make_provider") as mock_prov:
...
def test_workspace_override(tmp_path):
    ...
    bot = Flyflor.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from flyflor.config.schema import Config
    from flyflor.flyflor import _make_provider
...
def test_import_from_top_level():
    ...
    assert flyflor.Flyflor is Flyflor
    assert flyflor.RunResult is RunResult
...
async def test_run_populates_tools_used_across_iterations(tmp_path):
    ...
    from flyflor.agent.hook import AgentHookContext
    from flyflor.bus.events import OutboundMessage
    from flyflor.providers.base import ToolCallRequest
