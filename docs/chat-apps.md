...
Connect flyflor to your favorite chat platform. Want to build your own? See the [Channel Plugin Guide](./channel-plugin-guide.md).

| Channel | What you need |
|---------|---------------|
| **Telegram** | Bot token from @BotFather |
| **Discord** | Bot token + Message Content intent |
| **WhatsApp** | QR code scan (`flyflor channels login whatsapp`) |
| **WeChat (Weixin)** | QR code scan (`flyflor channels login weixin`) |
...
**1. Ask flyflor to set up Mochat for you**

Simply send this message to flyflor (replace `xxx@xxx` with your real email):

```
Read https://raw.githubusercontent.com/Flyflor/MoChat/refs/heads/main/skills/flyflor/skill.md and register on MoChat. My Email account is xxx@xxx Bind me as your owner and DM me on MoChat.
```

flyflor will automatically register, configure `~/.flyflor/config.json`, and connect to Mochat.
...
That's it — flyflor handles the rest!
...
If you prefer to configure manually, add the following to `~/.flyflor/config.json`:
...
- You need:
  - `userId` (example: `@flyflor:matrix.org`)
  - `password`
...
```bash
flyflor channels login whatsapp
...
> After upgrading flyflor, rebuild the local bridge with:
> `rm -rf ~/.flyflor/bridge && flyflor channels login whatsapp`
...
- **Permissions**:
  ...
  - **Streaming replies** (default in flyflor): add **`cardkit:card:write`** (often labeled **Create and update cards** in the Feishu developer console). Required for CardKit entities and streamed assistant text. Older apps may not have it yet — open **Permission management**, enable the scope, then **publish** a new app version if the console requires it.
  - If you **cannot** add `cardkit:card:write`, set `"streaming": false` under `channels.feishu` (see below). The bot still works; replies use normal interactive cards without token-by-token streaming.
- **Events**: Add `im.message.receive_v1` (receive messages)
  - Select **Long Connection** mode (requires running flyflor first to establish connection)
...
> `allowFrom`: Add your open_id (find it in flyflor logs when you message the bot). Use `["*"]` to allow all users.
...
> - `allowFrom`: Add your openid (find it in flyflor logs when you message the bot). Use `["*"]` for public access.
...
> `files:read` is required to read files users send to flyflor. `files:write` is required for flyflor to send images, videos, and other file uploads. If you add either scope later, reinstall the Slack app to the workspace and restart flyflor so it uses the updated bot token.
...
Give flyflor its own email account. It polls **IMAP** for incoming mail and replies via **SMTP** — like a personal email assistant.

**1. Get credentials (Gmail example)**
- Create a dedicated Gmail account for your bot (e.g. `my-flyflor@gmail.com`)
...
```json
{
  "channels": {
    "email": {
      ...
      "imapPort": 993,
      "imapUsername": "my-flyflor@gmail.com",
      "imapPassword": "your-app-password",
      "smtpHost": "smtp.gmail.com",
      "smtpPort": 587,
      "smtpUsername": "my-flyflor@gmail.com",
      "smtpPassword": "your-app-password",
      "fromAddress": "my-flyflor@gmail.com",
...
> - `allowFrom`: Add the sender ID you see in flyflor logs for your WeChat account. Use `["*"]` to allow all users.
> - `token`: Optional. If omitted, log in interactively and flyflor will save the token for you.
> - `routeTag`: Optional. When your upstream Weixin deployment requires request routing, flyflor will send it as the `SKRouteTag` header.
> - `stateDir`: Optional. Defaults to flyflor's runtime directory for Weixin state.
> - `pollTimeout`: Optional long-poll timeout in seconds.

**3. Login**

```bash
flyflor channels login weixin
```

Use `--force` to re-authenticate and ignore any saved token:

```bash
flyflor channels login weixin --force
```
...
```bash
pip install flyflor-ai[msteams]
```
...
> - `mentionOnlyResponse` controls what Flyflor receives when a user sends only a bot mention (`<at>Flyflor</at>`). Set to `""` to ignore mention-only messages.
