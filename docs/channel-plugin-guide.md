...
Build a custom flyflor channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of flyflor (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

flyflor discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `flyflor gateway` starts, it scans:

1. Built-in channels in `flyflor/channels/`
2. External packages registered under the `flyflor.channels` entry point group
...
```text
flyflor-channel-webhook/
├── flyflor_channel_webhook/
...
```python
# flyflor_channel_webhook/__init__.py
from flyflor_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# flyflor_channel_webhook/channel.py
import asyncio
...
from flyflor.config.schema import Base
...
# pyproject.toml
[project]
name = "flyflor-channel-webhook"
version = "0.1.0"
dependencies = ["flyflor-ai", "aiohttp"]

[project.entry-points."flyflor.channels"]
webhook = "flyflor_channel_webhook:WebhookChannel"
...
packages = ["flyflor_channel_webhook"]
```
...
```bash
pip install -e .
flyflor plugins list      # verify "Webhook" shows as "plugin"
flyflor onboard           # auto-adds default config for detected plugins
```
...
```bash
flyflor channels login <channel_name>
flyflor channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `flyflor onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `flyflor.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `flyflor.config.schema.Base`:
...
Override `default_config()` so `flyflor onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `flyflor-channel-{name}` | `flyflor-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `flyflor_channel_{name}` | `flyflor_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/flyflor-channel-webhook
cd flyflor-channel-webhook
pip install -e .
flyflor plugins list    # should show "Webhook" as "plugin"
flyflor gateway         # test end-to-end
```
