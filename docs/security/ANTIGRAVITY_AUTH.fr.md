...
**Important :** Fournissez ces identifiants via des variables d'environnement ; ne validez pas d'identifiants OAuth dans le dépôt :

```typescript
const CLIENT_ID = process.env.FLYFLOR_GOOGLE_ANTIGRAVITY_CLIENT_ID ?? "<your-google-oauth-client-id>";
const CLIENT_SECRET = process.env.FLYFLOR_GOOGLE_ANTIGRAVITY_CLIENT_SECRET ?? "<your-google-oauth-client-secret>";
```
...
Si votre fournisseur nécessite OAuth ou une authentification spéciale, ajoutez un cas dans `cmd/flyflor/internal/auth/helpers.go` :
...
- **Fichiers source :**
  ...
  - `cmd/flyflor/internal/auth/helpers.go` - Commandes CLI d'authentification
