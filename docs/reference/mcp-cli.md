...
Flyflor includes an `mcp` CLI command group for managing MCP server entries in `config.json`.
...
The CLI updates the same config file used by the rest of Flyflor:
...
- saves atomically
- preserves the standard 2-space JSON formatting used by Flyflor
- validates the generated JSON before writing

Behavior notes:

- `flyflor mcp add ...` enables `tools.mcp.enabled`
- removing the last server with `flyflor mcp remove ...` disables `tools.mcp.enabled`
...
```bash
flyflor mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /tmp
```

Add a stdio server with environment variables saved in config:

```bash
flyflor mcp add github --env GITHUB_PERSONAL_ACCESS_TOKEN=ghp_xxx -- npx -y @modelcontextprotocol/server-github
```

Add a stdio server using an env file for secrets:

```bash
flyflor mcp add github --env-file .env.github -- npx -y @modelcontextprotocol/server-github
```

Add a remote HTTP server:

```bash
flyflor mcp add context7 --transport http https://mcp.context7.com/mcp
```

Add a remote HTTP server with auth header, even with flags after the URL:

```bash
flyflor mcp add apify "https://mcp.apify.com/" -t http --header "Authorization: Bearer OMITTED"
```

Add a stdio server using an explicit command separator:

```bash
flyflor mcp add --transport stdio --env AIRTABLE_API_KEY=YOUR_KEY airtable -- npx -y airtable-mcp-server
```
...
| Command | Purpose |
|---------|---------|
| `flyflor mcp add <name> [flags] <command-or-url> [args...]` | Add or update an MCP server entry |
| `flyflor mcp remove <name>` | Remove a server entry from config |
| `flyflor mcp list` | List configured MCP servers |
| `flyflor mcp show <name>` | Show full details and tools for one server |
| `flyflor mcp test <name>` | Try connecting to one configured server |
| `flyflor mcp edit` | Open `config.json` in `$EDITOR` |
...
```bash
flyflor mcp add <name> [flags] <command-or-url> [args...]
```
...
```bash
flyflor mcp add [flags] <name> <command-or-url> [args...]
flyflor mcp add [flags] <name> -- <command> [args...]
```
...
- use the `--` separator when the stdio command itself has arguments that may look like Flyflor CLI flags
- without `--`, Flyflor treats the first two non-flag tokens as `<name>` and `<command-or-url>`
...
```bash
flyflor mcp add sqlite npx -y @modelcontextprotocol/server-sqlite --db ./mydb.db
```
...
```bash
flyflor mcp add --deferred sqlite npx -y @modelcontextprotocol/server-sqlite --db ./mydb.db
```
...
- if `<name>` already exists, Flyflor asks for confirmation
- use `--force` to skip the prompt

Local path validation:

- if the command looks like a local path such as `./server.py` or `/opt/mcp/server`
- Flyflor checks that the file exists
- on non-Windows platforms, it also checks that the file is executable

Clear URL/transport error:

- if the target looks like `https://...` but transport is still `stdio`, Flyflor returns an explicit error telling you to use `--transport http` or `--transport sse`
...
```bash
flyflor mcp remove <name>
```

This removes the named entry from `tools.mcp.servers`.

If the removed server was the last configured MCP server, Flyflor also disables `tools.mcp.enabled`.
...
- without `--status`, Flyflor prints configuration state only
- with `--status`, Flyflor tries to connect to each enabled server and reports `ok (N tools)` or `error`
- to see the full list of tools a server exposes, use `flyflor mcp show <name>`
...
```bash
flyflor mcp show <name>
flyflor mcp show <name> --timeout 15s
```
...
Use it when you need to configure MCP fields that are not exposed directly by `flyflor mcp add`.
...
1. Add the server with `flyflor mcp add` (include `--deferred` if you want tools hidden by default).
2. Verify connectivity and inspect the exposed tools with `flyflor mcp show <name>`.
3. Check all servers at a glance with `flyflor mcp list --status`.
4. Start Flyflor normally so the configured MCP server is loaded by the host.

For advanced cases:

1. Add the base entry with `flyflor mcp add`.
2. Run `flyflor mcp edit` to fill in fields that are not exposed as CLI flags.
3. Run `flyflor mcp show <name>` to confirm the final configuration and tool list.
