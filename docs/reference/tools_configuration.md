...
Flyflor's tools configuration is located in the `tools` field of `config.json`.
...
Before tool results are sent to the LLM, Flyflor can filter sensitive values (API keys, tokens, secrets) from the output. This prevents the LLM from seeing its own credentials.
...
If `range` is omitted, Flyflor performs an unrestricted search.
...
By default, Flyflor blocks the following dangerous commands:
...
The exec guard only validates the top-level command sent to Flyflor. It does **not** recursively inspect child
...
If you prefer not to edit JSON manually, Flyflor also provides an MCP configuration manager CLI:

- `flyflor mcp add` — add or update a server (supports `--deferred` / `--no-deferred`)
- `flyflor mcp list` — list all configured servers with status and deferred state
- `flyflor mcp show <name>` — show full details and the tool list for one server
- `flyflor mcp test <name>` — connectivity check for one server
- `flyflor mcp remove <name>` — remove a server entry
- `flyflor mcp edit` — open `config.json` in `$EDITOR` for advanced edits
