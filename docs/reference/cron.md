...
Flyflor stores scheduled jobs in the current workspace and can run them either as reminders, full agent turns, or shell commands.

## Schedule Types

Flyflor currently uses three schedule forms in the cron tool:
...
The CLI command `flyflor cron add` currently supports recurring jobs only:
...
```bash
flyflor cron add --name "Daily summary" --message "Summarize today's logs" --cron "0 18 * * *"
flyflor cron add --name "Ping" --message "heartbeat" --every 300 --deliver
```
...
When the job fires, Flyflor sends the saved message back through the agent loop as a new agent turn. Use this for scheduled work that may need reasoning, tools, or a generated reply.

### `deliver: true`

When the job fires, Flyflor publishes the saved message directly to the target channel and recipient without agent processing.

The CLI `flyflor cron add --deliver` flag uses this mode.

### `command`

When a cron-tool job includes `command`, Flyflor runs that shell command through the `exec` tool and publishes the command output back to the channel.
...
The current CLI `flyflor cron add` command does not expose a `command` flag.
...
This is not a hard disable switch. If you set `allow_command` to `false`, Flyflor still allows a command job when the caller explicitly passes `command_confirm: true`.
...
Both the gateway and `flyflor cron` CLI subcommands use the same `cron/jobs.json` file.

Notes:

- one-time `at_seconds` jobs are deleted after they run
- recurring jobs stay in the store until removed
- disabled jobs stay in the store and still appear in `flyflor cron list`
