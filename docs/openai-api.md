...
flyflor can expose a minimal OpenAI-compatible endpoint for local integrations:

```bash
pip install "flyflor-ai[api]"
flyflor serve
```
...
If `channel` points to a channel that is not enabled in your config, flyflor will queue the outbound event but no platform delivery will occur.
