.PHONY: all build install uninstall clean help test build-all lint-docs dev-webui build-web-dist dev-build-web-backend dev-build-cli
...
## dev-webui: Run the Web UI with Vite HMR and the existing Docker backend, without rebuilding Docker
dev-webui:
	@./scripts/dev-webui.sh

## build-web-dist: Build only the frontend dist for a mounted Web backend, without rebuilding Docker
build-web-dist:
	@./scripts/build-web-dist.sh

## dev-build-web-backend: Incrementally build flyflor-web in a reusable builder container and copy it into the running container
dev-build-web-backend:
	@./scripts/dev-build-web-backend.sh

## dev-build-cli: Incrementally build the flyflor CLI and copy it into the running container
dev-build-cli:
...
## docker-build: Build the Flyflor Docker image
docker-build:
	@echo "Building Flyflor Docker image..."
	docker compose build flyflor

## docker-run: Run Flyflor and Qdrant with the root Docker Compose file
docker-run:
	docker compose up -d flyflor

## docker-logs: Follow Flyflor Docker logs
docker-logs:
	docker compose logs -f flyflor

## docker-run-agent: Run Flyflor agent once in Docker
docker-run-agent:
	docker compose run --rm flyflor $(ARGS)
...
help:
	...
	@echo "  make docker-run         # Run Flyflor with Docker Compose"
	@echo "  make dev-webui          # Run Web UI with Vite HMR"
	@echo ""
