    ...
    let checks = vec![
        HealthCheck {
            ...
            fix_hint: if config_path.exists() {
                ...
                Some("Run `flyflor onboard` or `flyflor doctor --fix`.".to_string())
            },
        },
        HealthCheck {
        ...
        HealthCheck {
            ...
            fix_hint: if workspace_ok {
                ...
                Some("Run `flyflor doctor --fix`.".to_string())
            },
        },
        HealthCheck {
            ...
            fix_hint: if has_any_provider(config) {
                ...
                Some("Set providers.*.apiKey in ~/.flyflor/config.json.".to_string())
            },
        },
        HealthCheck {
        ...
        HealthCheck {
            ...
            fix_hint: if cron_count == 0 {
                Some("Use `flyflor cron add ...` if you rely on automation.".to_string())
            } else {
                None
            },
        },
...
pub async fn check_update(crate_name: &str) -> Result<UpdateReport> {
    ...
    let response = reqwest::Client::new()
        .get(url)
        .header("User-Agent", "flyflor-update-check")
        .send()
        .await;
    match response {
