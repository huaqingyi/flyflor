...
const AVATAR_PNG: &[u8] = include_bytes!("flyflor-avatar.png");
    ...
    fn new() -> Self {
        ...
        std::thread::spawn(move || {
            ...
            if api_key.is_none() && !is_bedrock {
                let err =
                    "No API key configured. Set providers.*.apiKey in ~/.flyflor/config.json."
                        .to_string();
...
fn respond_bytes(req: Request, status: u16, content_type: &str, body: &'static [u8]) {
...
fn handle_request(mut req: Request, ctx: &WebUiContext) {
    ...
    match (method, url.as_str()) {
        ...
        (Method::Get, "/flyflor-avatar.png") => respond_bytes(req, 200, "image/png", AVATAR_PNG),
