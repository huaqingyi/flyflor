...
impl Default for AgentDefaults {
    fn default() -> Self {
        Self {
            workspace: "~/.flyflor/workspace".to_string(),
        ...
        match name {
            ...
            Some("fastai") => Some(
                self.providers
                    ...
                    .api_base
                    .clone()
                    .unwrap_or_else(|| "https://fastai.fast/v1".to_string()),
            ),
            Some("aihubmix") => Some(
                self.providers
                    .aihubmix
...
pub fn providers_status(config: &Config) -> Map<String, Value> {
    ...
    map.insert(
        "fastai".to_string(),
        Value::Bool(!config.providers.fastai.api_key.is_empty()),
    );
    map.insert(
