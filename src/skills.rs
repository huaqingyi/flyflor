...
impl SkillsLoader {
        ...
        if filter_unavailable {
            skills
                ...
                .filter(|skill| {
                    ...
                    let parsed = parse_flyflor_metadata(
    ...
    pub fn build_skills_summary(&self) -> String {
        ...
        for skill in skills {
            ...
            let skill_meta = parse_flyflor_metadata(
                meta.get("metadata").map(String::as_str).unwrap_or_default(),
            );
...
fn parse_flyflor_metadata(raw: &str) -> Value {
    if raw.is_empty() {
        return Value::Object(Default::default());
    }
    serde_json::from_str::<Value>(raw)
        .ok()
        .and_then(|v| v.get("flyflor").cloned().or(Some(v)))
