...
const PROVIDERS: &[ProviderSpec] = &[
    ProviderSpec {
        ...
        detect_by_base_keyword: "fastai.fast",
        default_api_base: "https://fastai.fast/v1",
...
impl LiteLLMProvider {
    ...
    fn openai_compat_model(&self, model: &str) -> String {
        ...
        let provider_prefix = format!("{}/", gateway.name);
        model
    ...
    fn set_env_var(key: &str, value: &str, overwrite: bool) {
        ...
        // to preserve LiteLLM provider compatibility.
...
impl LLMProvider for LiteLLMProvider {
    async fn chat(
        ...
        if self.use_openai_compat_path(selected_model) {
            let compat_model = self.openai_compat_model(selected_model);
...
mod tests {
    ...
    fn resolve_model_applies_gateway_and_provider_rules() {
        let fastai = LiteLLMProvider::new(
            "x",
            Some("https://fastai.fast/v1".to_string()),
            "fastai/gpt-5.5",
            None,
            Some("fastai"),
        );
        assert_eq!(fastai.openai_compat_model("fastai/gpt-5.5"), "gpt-5.5");
