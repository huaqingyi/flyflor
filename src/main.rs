...
use flyflor::VERSION;
use flyflor::agent::AgentLoop;
use flyflor::bus::{MessageBus, OutboundMessage};
use flyflor::channels::manager::ChannelManager;
use flyflor::config::{Config, get_config_path, load_config, providers_status, save_config};
use flyflor::cron::{CronSchedule, CronService};
use flyflor::health::{CheckLevel, HealthReport, check_update, collect_health, run_doctor};
use flyflor::heartbeat::{DEFAULT_HEARTBEAT_INTERVAL_S, HeartbeatService};
use flyflor::pairing::{approve_pairing, list_pending, reject_pairing};
use flyflor::providers::base::LLMProvider;
use flyflor::providers::litellm::LiteLLMProvider;
use flyflor::service::{self, ServiceAccount, ServiceInstallOptions};
use flyflor::session::SessionManager;
use flyflor::tools::cron::CronTool;
use flyflor::utils::{get_data_path, get_workspace_path};
use flyflor::webui::run_webui_server;
use std::fs;
use std::io::BufRead;
...
#[command(
    name = "flyflor",
    about = "Flyflor (飞花): observable AI agent runtime"
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Version => println!("flyflor v{VERSION}"),
...
fn cmd_onboard() -> Result<()> {
    ...
    let templates = [
        ...
        (
            "SOUL.md",
            "# Soul\n\nI am Flyflor (飞花), an observable AI agent runtime with tools, memory, scheduling, and multi-channel messaging.\n",
        ),
        (
            "USER.md",
    ...
    println!("flyflor is ready.");
    println!("Next steps:");
    println!("1. Add your API key to {}", config_path.display());
    println!("2. Chat: flyflor agent -m \"Hello!\"");
    Ok(())
}
...
async fn cmd_update() -> Result<()> {
    let report = check_update("flyflor").await?;
    ...
    if report.update_available {
        ...
        println!("Hint: run `cargo install flyflor --locked --force`");
    } else {
...
async fn cmd_agent(message: Option<String>, session: &str) -> Result<()> {
    ...
    if api_key.is_none() && !is_bedrock {
        ...
        println!("Set one in ~/.flyflor/config.json under providers.*.apiKey");
        return Ok(());
    }
    ...
    if let Some(content) = message {
        let response = agent_loop
            ...
            .await?;
        println!("flyflor: {response}");
    } else {
        println!("flyflor interactive mode (type exit/quit or Ctrl+C to exit)");
...
fn resolve_service_name(config: &Config, name: Option<&str>) -> Result<String> {
    let resolved = name
        ...
        .unwrap_or_else(|| "FlyflorService".to_string());
...
fn resolve_install_account(
    ...
    if use_current_user {
        let resolved_password = password
            .or_else(|| std::env::var("FLYFLOR_SERVICE_PASSWORD").ok())
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty())
            .ok_or_else(|| {
                anyhow!(
                    "missing password: use --password or set FLYFLOR_SERVICE_PASSWORD when --use-current-user is enabled"
                )
            })?;
...
fn cmd_service(command: ServiceCommand) -> Result<()> {
    ...
    match command {
        ServiceCommand::Install {
            ...
            println!("Use `flyflor service start` to start it.");
        }
...
fn cmd_pairing(command: PairingCommand) -> Result<()> {
    match command {
        ...
        PairingCommand::Approve { channel, code } => {
            ...
            println!(
                "Sender added to allowlist. You can verify with `flyflor channels status` or config.json."
            );
        }
...
fn find_bridge_source() -> Option<PathBuf> {
    ...
    if let Ok(cwd) = std::env::current_dir() {
        ...
        candidates.push(cwd.join("..").join("flyflor").join("bridge"));
    }
...
async fn cmd_cron(command: CronCommand) -> Result<()> {
    ...
    match command {
        ...
        CronCommand::Run { job_id, force } => {
            ...
            if api_key.is_none() && !is_bedrock {
                return Err(anyhow!(
                    "No API key configured. Set one in ~/.flyflor/config.json under providers.*.apiKey"
                ));
            }
