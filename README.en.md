...
`flyflor` is an observable agent runtime for personal and multi-channel workflows. It is built around tool execution, session memory, scheduled work, and channel gateways so Flyflor can stay consistent across chat, automation, and long-running context.

- Rust implementation with stable concurrency and straightforward deployment
- Agent identity: Flyflor / 飞花, calm, precise, direct, and capable of long-term memory plus tool collaboration
- Runtime focus: an observable agent loop, not just a chat client
- Evolution path: reflection, layered memory, blackboard collaboration, complexity routing, and multi-channel gateways

## Architecture Direction

See [DESIGN.md](DESIGN.md) for the product architecture. The current Rust implementation keeps the lightweight runtime core while moving toward Flyflor's memory, reflection, blackboard, and observability design.

## ✨ Features

- Agent loop: LLM calls, tool execution, session context, and error handling
- Config system: `~/.flyflor/config.json` with provider auto-matching
...
- Built-in project skills (`skills/*`)
- Ops and maintenance:
...
`flyflor` uses LiteLLM-style model routing. You can set the model directly (no `litellm/` prefix required), for example:
...
`flyflor` can run as a Windows background service via `nssm`, with built-in commands:
...
Install service (default service name: `FlyflorService`, default args: `gateway`):
...
When `--name` is provided, the value is persisted to `service.name` in `~/.flyflor/config.json`, so later `start/stop/status` can omit `--name`.
...
2. Use current user (recommended, easier access to your user-scoped `~/.flyflor/config.json`):

```powershell
.\target\release\flyflor.exe service install --use-current-user --password "YourWindowsPassword"
```

Or use environment variable to avoid plaintext password in command history:

```powershell
$env:FLYFLOR_SERVICE_PASSWORD="YourWindowsPassword"
...
Disabled by default. Once enabled, flyflor uses HTTP watch/polling to receive and send messages.

1. Optional: ask flyflor to set up Mochat automatically
...
- flyflor will try to register and write Mochat settings into `~/.flyflor/config.json`.

2. Manual setup (recommended to verify config)
- Configure `channels.mochat` in `~/.flyflor/config.json`:
...
3. Configure `~/.flyflor/config.json`
